...
CALLBACK_URL = "http://localhost:8000/api/minew"
...
# 1add08 = 24 = alhopuro-vasen
# 1add09 = 78 = koti2-vasen
# 1add10 = ?? = kolmas
...
async def send_payload_task(deviceIdentifier):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            await client.post(CALLBACK_URL, json=payload, timeout=3)
        print(payload)
    finally:
...
class DeviceState():
    ...
    def on_msgnum(self, msgnum):
        ...
        if prev is None:
            logger.info(f"Device {self.deviceIdentifier}: First msgnum {msgnum} received")
            return
        if msgnum<prev:
            logger.info(f"Device {self.deviceIdentifier}: New msgnum {msgnum} wrapped around")
            prev=prev-255
        if msgnum-prev > 1:
            logger.warning(f"Device {self.deviceIdentifier}: Msgnum {msgnum} is {msgnum-prev} higher than previous {prev}, missed messages?")
...
async def proc(bd: BLEDevice, ad: AdvertisementData,
    ...
    logger.info(f"payload parsed: enters={parsed.enters} exit={parsed.exits} msgc={parsed.msgcounter} id={deviceIdentifier} TOTAL(ENTER={device.total_enter} EXIT={device.total_exit})",enters=parsed.enters,
                exits=parsed.exits,
                msgcounter=parsed.msgcounter,
